...
$query3 = mysqli_query($db, "SELECT `Date` FROM `covid_cases` ORDER BY `covid_cases`.`Date` DESC");
$array1 = array();
if (mysqli_num_rows($query3) > 0) {
	while($row = mysqli_fetch_assoc($query3)) {
		array_push($array1,$row['Date']);
	}
}
...
for($i=0; $i<count($array1); $i++){
	...
	$query4 = mysqli_query($db, "SELECT `visit_id` FROM `user_visits` WHERE `Date_of_upload` BETWEEN DATE('$date2') AND DATE('$date3')");
